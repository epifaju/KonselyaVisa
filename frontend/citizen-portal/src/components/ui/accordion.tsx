...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-3 px-4 py-3 text-left text-body font-medium text-foreground transition-all hover:bg-muted/60 [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
